...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self,
        ...
        if self.middle_layer_size != 0:
            self.middle_classifier = nn.Linear(self.reduced_dim + self.desc_conv_output_size*2 + \
                                               self.formula_conv_output_size*2 + len(self.conv_window_size)*self.out_conv_list_dim,
            ...
            self.classifier = nn.Linear(self.reduced_dim + self.desc_conv_output_size*2 + \
                                               self.formula_conv_output_size*2 + len(self.conv_window_size)*self.out_conv_list_dim, self.num_labels)
            self._init_weights(self.classifier)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        # Fusion
        pooled_output = torch.cat((pooled_output, formula_output, desc_outputs, image_classifier_output1), 1)
...
class Trainer:
    ...
    def train(self, training_loader, validation_loader, num_epochs,
                    ...
                    modal1='formula', modal2='desc', **kwargs):
        """ Train the model """
        print("modal1='formula', modal2='desc' and image.\n Graph, formula separate into 2 entities as normally.\n Image is a set of 2000-dim features.")
        train_loss = 0.0
        ...
        for epoch in range(num_epochs):
            ...
            epoch_iterator = zip(
                ...
                train_mol4_loader,
                train_image_loader)
            for step, (batch, mol1, mol2, mol3, mol4, image) in enumerate(tqdm(epoch_iterator)):
                nb_train_step += 1
                self.model.train()
    ...
    def evaluate(self, 
        ...
        preds = None
        out_label_ids = None
        for (batch, mol1, mol2, mol3, mol4, image) in tqdm(zip(validation_loader, test_mol1_loader, test_mol2_loader, test_mol3_loader, test_mol4_loader, test_image_loader)):
            self.model.eval()
            batch = tuple(t.to(self.device) for t in batch)
            with torch.no_grad():
                inputs = {'input_ids':      batch[0],
