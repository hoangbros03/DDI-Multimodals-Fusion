...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self,
        ...
        if self.middle_layer_size == 0:
            self.classifier = nn.Linear(2*self.formula_conv_output_size, 5)
        else:
            self.middle_classifier = nn.Linear(2*self.formula_conv_output_size, self.middle_layer_size)
    ...
    def forward(self, 
        ...
        pooled_output = torch.cat((pooled_formula1_output, pooled_formula2_output), 1)
        ...
        if labels is not None:
            ...
            elif self.data_type == "double_effect":
                weight = torch.tensor([1., 1., 5., 1., 1.]).to('cuda' if torch.cuda.is_available() else 'cpu')
                loss_fct = CrossEntropyLoss(weight=weight)
            else:
