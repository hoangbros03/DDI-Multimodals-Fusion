...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self,
                ...
                apply_mask=True,
                represent_dim = 24, # Graph, formula, desc, image
                modal="desc",
                **kwargs
                ):
        if 'seed' in kwargs:
        ...
        print("Represent dim: ", represent_dim)
        ...
        if self.modal == "graph":
            ...
            self.graph_classifier_represent = nn.Linear(self.num_labels, self.represent_dim)
            self.norm_graph = torch.nn.BatchNorm1d(self.represent_dim)
        ...
        if self.modal == "formula":
            ...
            self.formula_classifier_represent = nn.Linear(self.num_labels, self.represent_dim)

            self.norm_formula = torch.nn.BatchNorm1d(num_features=self.represent_dim)
        ...
        if self.modal == "desc":
            ...
            self.desc_classifier_represent = nn.Linear(self.num_labels, self.represent_dim)
            self.norm_desc = torch.nn.BatchNorm1d(num_features=self.represent_dim)
        ...
        if self.modal == "image":
            ...
            self.image_classifier = nn.Linear(self.reduced_dim, self.num_labels)
            self.image_classifier_represent = nn.Linear(self.num_labels, self.represent_dim)
            self.norm_img = nn.BatchNorm1d(num_features = self.represent_dim)
        ...
        if self.middle_layer_size != 0:
            self.middle_classifier = nn.Linear(self.represent_dim + len(self.conv_window_size)*self.out_conv_list_dim, self.middle_layer_size)
            self.classifier = nn.Linear(self.middle_layer_size, self.num_labels)
        else:
            self.classifier = nn.Linear(self.represent_dim + len(self.conv_window_size)*self.out_conv_list_dim, self.num_labels)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        if self.modal == "graph":
            ...
            gnn_predict = self.graph_classifier(\
                            self.modal_dropout(torch.cat((gnn1_outputs, gnn2_outputs), 1)))
            gnn_represent = self.graph_classifier_represent(self.activation(gnn_predict))
            gnn_represent = self.norm_graph(gnn_represent)
            modal_represent = gnn_represent
            modal_predict = gnn_predict
        ...
        if self.modal == "formula":
            ...
            formula_outputs = self.modal_dropout(torch.cat((pooled_formula1_output, pooled_formula2_output), 1))
            ...
            formula_represent = self.norm_formula(formula_represent)
            modal_represent = formula_represent
            modal_predict = formula_predict
    ...
    #         pooled_desc2_output = self.norm_desc(pooled_desc2_output)
            ...
            desc_predict = self.desc_classifier(desc_outputs)
            desc_represent = self.desc_classifier_represent(self.activation(desc_predict))
            desc_represent = self.norm_desc(desc_represent)
            modal_represent = desc_represent
            modal_predict = desc_predict

        # Image
        if self.modal == "image":
            image_classifier_output1 = self.activation(self.reduce_img_dim_layer(image_classifier_output1))
            image_outputs = self.modal_dropout(image_classifier_output1)
            image_predict = self.image_classifier(image_outputs)
            image_represent = self.image_classifier_represent(self.activation(image_predict))
            image_represent = self.norm_img(image_represent)
            modal_represent = image_represent
            modal_predict = image_predict

        # Late fusion
        pooled_output = torch.cat((pooled_output, modal_represent), 1)
        ...
        outputs = (loss,) + outputs + (modal_predict,)
        ...
        for epoch in range(num_epochs):
            ...
            if self.modal == "image":
                ...
                epoch_iterator = zip(
                    ...
                    train_mol1_loader, 
                    train_mol2_loader)
            for step, iter_data in enumerate(tqdm(epoch_iterator)):
#                 (batch, mol1, mol2, mol3, mol4, mol5, mol6, image)
                if self.modal == "image":
                    batch, image = iter_data
                else:
                    batch, mol1, mol2 = iter_data
                nb_train_step += 1
                self.model.train()
    ...
    def evaluate(self, 
        ...
        out_label_ids = None
        if self.modal == "image":
            test_loader = zip(validation_loader, test_image_loader)
        else:
            test_loader = zip(validation_loader, test_mol1_loader, test_mol2_loader)
        
        for iter_data in tqdm(test_loader):
#             (batch, mol1, mol2, mol3, mol4, mol5, mol6, image)
            if self.modal =="image":
                batch, image = iter_data
            else:
            ...
            if preds is None:
                ...
                modals_preds = [outputs[-1].detach().cpu().numpy()]
                out_label_ids = inputs['labels'].detach().cpu().numpy()
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                modals_preds = [np.append(modals_preds[i], outputs[i-1].detach().cpu().numpy(), axis=0) for i in range(1)]
                out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy(), axis=0)
            
        eval_loss = eval_loss / nb_eval_steps
        to_save_pred = preds
        preds = np.argmax(preds, axis=1)
        modals_preds = [np.argmax(modals_preds[i], axis=1) for i in range(1)]
...
#         import pdb; pdb.set_trace()
        ...
        modals_preds = [self.convert_prediction_to_full_prediction(modals_preds[i], filtered_lst_index, full_label) for i in range(1)]
        modals_result = [self.ddie_compute_metrics(modals_preds[i], full_label) for i in range(1)]
        ...
        print("With modal predict:")
        print(self.modal)
        pprint.pprint(modals_result[0])
