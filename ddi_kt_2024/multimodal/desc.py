...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self,
        ...
        print("Dropout text will be 0.5, while dropout of other modal is what you passed into this constructor")
        ...
        if self.middle_layer_size != 0:
            self.middle_classifier = nn.Linear(self.desc_conv_output_size*2 + \
                                              len(self.conv_window_size)*self.out_conv_list_dim,
                                              self.middle_layer_size)
            self.classifier = nn.Linear(self.middle_layer_size, self.num_labels)
        else:
            self.classifier = nn.Linear(self.desc_conv_output_size*2 + \
                                              len(self.conv_window_size)*self.out_conv_list_dim, self.num_labels)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        pooled_output = self.norm_text(torch.cat(conv_outputs, 1))
        pooled_output = self.dropout_bert_output(pooled_output)
        ...
        pooled_desc_output = self.modal_dropout(torch.cat((pooled_desc1_output, pooled_desc2_output), 1))
