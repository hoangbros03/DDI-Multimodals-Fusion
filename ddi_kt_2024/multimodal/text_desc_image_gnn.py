...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self,
        ...
        if self.middle_layer_size != 0:
            self.middle_classifier = nn.Linear(self.reduced_dim + self.desc_conv_output_size*2 + \
                                               self.gnn.hidden_channels*2 + len(self.conv_window_size)*self.out_conv_list_dim,
            ...
            self.classifier = nn.Linear(self.reduced_dim + self.desc_conv_output_size*2 + \
                                               self.gnn.hidden_channels*2 + len(self.conv_window_size)*self.out_conv_list_dim, self.num_labels)
            self._init_weights(self.classifier)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        # Fusion
        pooled_output = torch.cat((pooled_output, desc_outputs, gnn_outputs, image_classifier_output1), 1)
...
class Trainer:
    ...
    def train(self, training_loader, validation_loader, num_epochs,
                    ...
                    modal1='graph', modal2='desc', **kwargs):
        """ Train the model """
        print("modal1='graph', modal2='desc' and image.\n Graph and description separate into 2 entities as normally.\n Image is a set of 2000-dim features.")
        train_loss = 0.0
