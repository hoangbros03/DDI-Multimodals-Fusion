...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(
        ...
        if self.middle_layer_size == 0:
            self.classifier = nn.Linear(self.out_conv_list_dim*len(self.conv_window_size)+2*self.gnn.hidden_channels+2*self.desc_conv_output_size, self.num_labels)
        else:
            self.middle_classifier = nn.Linear(self.out_conv_list_dim*len(self.conv_window_size)+2*self.gnn.hidden_channels+2*self.desc_conv_output_size, self.middle_layer_size)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        # Fusion
        pooled_output = torch.cat((pooled_output, desc_outputs, gnn_outputs), 1)
