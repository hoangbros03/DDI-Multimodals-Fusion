...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(
        ...
        if self.middle_layer_size == 0:
            self.classifier = nn.Linear(self.out_conv_list_dim*len(self.conv_window_size) + 2*self.formula_conv_output_size + 2*self.desc_conv_output_size, config.num_labels)
        else:
            self.middle_classifier = nn.Linear(self.out_conv_list_dim*len(self.conv_window_size) + 2*self.formula_conv_output_size + 2*self.desc_conv_output_size, self.middle_layer_size)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        # Fusion
        pooled_output = torch.cat((pooled_output, desc_outputs, formula_output), 1)
...
class Trainer:
    ...
    def train(self, training_loader, validation_loader, num_epochs,
        ...
        for epoch in range(num_epochs):
            print(f"================Epoch {epoch}================")
            epoch_iterator = zip(training_loader, train_mol1_loader, train_mol2_loader, train_mol3_loader, train_mol4_loader)
            for step, (batch, mol1, mol2, mol3, mol4) in tqdm(enumerate(epoch_iterator)):
                nb_train_step += 1
                self.model.train()
    ...
    def evaluate(self, 
        ...
        preds = None
        out_label_ids = None
        for (batch, mol1, mol2, mol3, mol4) in zip(validation_loader, test_mol1_loader, test_mol2_loader, test_mol3_loader, test_mol4_loader):
            self.model.eval()
