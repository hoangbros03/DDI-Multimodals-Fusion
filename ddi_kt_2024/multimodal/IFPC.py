    ...
    def __init__(self,
        ...
        if self.fusion_module.lower() == "tensorfusion":
            ...
            if self.middle_layer_size != 0:
                self.middle_classifier = nn.Linear(self.out_features_tensorfusion*2 + len(self.conv_window_size)*self.out_conv_list_dim, self.middle_layer_size)
                self._init_weights(self.middle_classifier)
                self.classifier = nn.Linear(self.middle_layer_size, self.num_labels)
            else:
                self.classifier = nn.Linear(self.out_features_tensorfusion*2 + len(self.conv_window_size)*self.out_conv_list_dim, self.num_labels)
        elif self.fusion_module.lower() == "hybridfusion":
            # Check if all modal enabled
            print("Using hybrid fusion. Please make sure that represent dim of other modals is equal with each other.")
            if not self.use_graph & self.use_desc & self.use_image & self.use_formula:
                raise ValueError("Not all modal used. Please check")
            if 'out_features_hybridfusion' not in kwargs:
                print("Out features of hybridfusion will be 48 due to no setting in kwargs")
                self.out_features_hybridfusion=48
            else:
                print("Out features of hybridfusion: ", kwargs['out_features_hybridfusion'])
                self.out_features_hybridfusion=kwargs['out_features_hybridfusion']
            self.TensorFusion_gd = TensorFusion(represent_dim=[len(self.conv_window_size)*self.out_conv_list_dim, self.represent_dim[0],self.represent_dim[2]],\
                                             apply_cnn=True, out_features=self.out_features_hybridfusion)
            self.TensorFusion_fi = TensorFusion(represent_dim=[len(self.conv_window_size)*self.out_conv_list_dim, self.represent_dim[1],self.represent_dim[3]],\
                                             apply_cnn=True, out_features=self.out_features_hybridfusion)
            self.text_for_elementwise = nn.Linear(len(self.conv_window_size)*self.out_conv_list_dim, self.represent_dim[0])
            self.weight_elementwise_tgd = nn.Linear(self.represent_dim[0],self.represent_dim[0])
            self.weight_elementwise_tfi = nn.Linear(self.represent_dim[0],self.represent_dim[0])
            self.binary_classifier = nn.Linear(self.out_features_hybridfusion*2 + len(self.conv_window_size)*self.out_conv_list_dim + self.represent_dim[0]*2, 2)
            
            self._init_weights(self.text_for_elementwise)
            self._init_weights(self.weight_elementwise_tgd)
            self._init_weights(self.weight_elementwise_tfi)
            self._init_weights(self.binary_classifier)

            # Output
            if self.middle_layer_size != 0:
                self.middle_classifier = nn.Linear(self.out_features_hybridfusion*2 + len(self.conv_window_size)*self.out_conv_list_dim + self.represent_dim[0]*2, self.middle_layer_size)
                self._init_weights(self.middle_classifier)
                self.classifier = nn.Linear(self.middle_layer_size, self.num_labels)
            else:
                self.classifier = nn.Linear(self.out_features_hybridfusion*2 + len(self.conv_window_size)*self.out_conv_list_dim + self.represent_dim[0]*2, self.num_labels)
