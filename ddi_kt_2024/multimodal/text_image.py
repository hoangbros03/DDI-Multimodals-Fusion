...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self,
        ...
        if self.middle_layer_size == 0:
            self.classifier = nn.Linear(self.out_conv_list_dim*len(self.conv_window_size)+ self.reduced_dim, config.num_labels)
            self._init_weights(self.classifier)
        else:
            self.middle_classifier = nn.Linear(self.out_conv_list_dim*len(self.conv_window_size)+ self.reduced_dim, self.middle_layer_size)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                ...
                labels=None, image_classifier_output1=None):
        ...
        # Fusion
        pooled_output = torch.cat((pooled_output, image_classifier_output1), 1)
...
class Trainer:
    ...
    def train(self, training_loader, validation_loader, num_epochs=None,
        ...
        for epoch in range(num_epochs):
            print(f"================Epoch {epoch}================")
            epoch_iterator = zip(training_loader, train_image_loader)
            for step, (batch, image) in tqdm(enumerate(epoch_iterator)):
                nb_train_step += 1
                self.model.train()
    ...
    def evaluate(self, 
        ...
        preds = None
        out_label_ids = None
        for (batch, image) in tqdm(zip(validation_loader, test_image_loader)):
            self.model.eval()
            batch = tuple(t.to(self.device) for t in batch)
            with torch.no_grad():
                inputs = {'input_ids':      batch[0],
